...
namespace IEC_UDINT_TESTS
{
    [TestClass]
    public class IEC_UDINT_BasicTests
    {
        [TestMethod]
        public void DefaultInitTest()
        {
            var variable1 = new IEC_UDINT();
            Assert.IsTrue(variable1.GetType() == typeof(IEC_UDINT));
            Assert.AreEqual(variable1.Value, (UInt32)0);

            IEC_UDINT variable2 = 127;
            Assert.IsTrue(variable2.GetType() == typeof(IEC_UDINT));
            Assert.AreEqual(variable2.Value, (UInt16)127);

            var variable3 = new IEC_UDINT() { Value = 90 };
            Assert.IsTrue(variable3.GetType() == typeof(IEC_UDINT));
