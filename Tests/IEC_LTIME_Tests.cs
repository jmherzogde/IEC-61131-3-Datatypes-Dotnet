...
namespace IEC_LTIME_TESTS
{
    [TestClass]
    public class IEC_LTIME_Tests
    {
        [TestMethod]
        public void DefaultInitTest()
        {
            var variable1 = new IEC_LTIME();
            Assert.IsTrue(variable1.GetType() == typeof(IEC_LTIME));
            Assert.AreEqual(variable1.Value, (UInt64)0);

            IEC_LTIME variable2 = 150;
            Assert.IsTrue(variable2.GetType() == typeof(IEC_LTIME));
            Assert.AreEqual(variable2.Value, (UInt64)150);

            var variable3 = new IEC_LTIME { Value = 90 };
            Assert.IsTrue(variable3.GetType() == typeof(IEC_LTIME));
