...
namespace IEC_WORD_TESTS
{
    [TestClass]
    public class IEC_WORD_BasicTests
    {
        [TestMethod]
        public void DefaultInitTest()
        {
            var variable1 = new IEC_WORD();
            Assert.IsTrue(variable1.GetType() == typeof(IEC_WORD));
            Assert.AreEqual(variable1.Value, (UInt16)0);

            IEC_WORD variable2 = 65000;
            Assert.IsTrue(variable2.GetType() == typeof(IEC_WORD));
            Assert.AreEqual(variable2.Value, (UInt16)65000);

            var variable3 = new IEC_WORD() { Value = 90 };
            Assert.IsTrue(variable3.GetType() == typeof(IEC_WORD));
